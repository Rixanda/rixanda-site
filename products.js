...
function renderProductPage() {
  ...
  fetch('data/products.json')
    ...
    .then(data => {
      ...
      if (p.cover_image) {
        imgWrap.innerHTML = `<img src="${p.cover_image}" alt="${p.name}">`;
      }

      const thumbsEl = document.getElementById('pp-thumbs');
      if (thumbsEl && p.gallery && p.gallery.length) {
        const allImages = [p.cover_image, ...p.gallery].filter(Boolean);
        thumbsEl.innerHTML = allImages.map((src, i) =>
          `<div class="pp-thumb${i === 0 ? ' active' : ''}" style="background-image:url('${src}');background-size:cover;background-position:center;cursor:pointer;" onclick="document.getElementById('pp-image-wrap').querySelector('img').src='${src}';document.querySelectorAll('.pp-thumb').forEach(t=>t.classList.remove('active'));this.classList.add('active');"></div>`
        ).join('');
      }
